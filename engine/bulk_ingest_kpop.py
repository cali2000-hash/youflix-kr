...
def bulk_ingest_kpop(target_count=500):
    """
    K-Pop 카테고리에 타겟 수량만큼 고품질 공식 영상을 수집하여 Firestore에 저장합니다.
    """
    ...
    print(f"🚀 [유플릭스 대량 수집] 카테고리: {category} | 목표 수량: {target_count}")
    ...
    for kw in keywords:
        ...
        per_kw_limit = min(50, remaining_needed) # 키워드당 수량 적절히 제한 (다양성 확보)
        ...
        if videos:
            ...
            for v in videos:
                if v['id'] not in all_collected_dict:
                    ...
                    new_added += 1
            print(f"✅ '{kw}'에서 {new_added}개의 신규 공식 영상 확보 (현재 총계: {len(all_collected_dict)})")
        else:
...
if __name__ == "__main__":
    # 명령어 인자로 수량 조절 가능 (예: python bulk_ingest_kpop.py 100)
    target = 500
    if len(sys.argv) > 1:
        try:
